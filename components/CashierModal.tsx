...
const CashierModal = (props: CashierModalProps) => {
  const [depositAmount, setDepositAmount] = useState("1");
  const [widthdrawAmount, setWidthdrawAmount] = useState("1");
  const [tabOpen, setTabOpen] = useState<"deposit" | "withdraw" | "history">(
    "deposit"
  );
  ...
  return (
    ...
    >
      <Popup isOpen={props.isOpen}>
        ...
        <TabSelectionRow>
          <TabSelectionButton
            active={tabOpen === "deposit"}
            onClick={() => setTabOpen("deposit")}
          >
            Deposit
          </TabSelectionButton>
          <TabSelectionButton
            active={tabOpen === "withdraw"}
            onClick={() => setTabOpen("withdraw")}
          >
            Withdraw
          </TabSelectionButton>
          <TabSelectionButton
            active={tabOpen === "history"}
        ...
        {tabOpen === "deposit" && (
          <ContentColumn>
            <SpacedRow>
              ...
              <ResetButton onClick={() => setDepositAmount("")}>
                Reset
              </ResetButton>
            </SpacedRow>
            <InputRow>
        ...
        {tabOpen === "withdraw" && (
          <ContentColumn>
            <SpacedRow>
              ...
              <ResetButton onClick={() => setWidthdrawAmount("")}>
                Reset
              </ResetButton>
            </SpacedRow>
            <InputRow>
            ...
            <Button
              onClick={() => props.onWithdraw(widthdrawAmount)}
              disabled={widthdrawAmount === "0" || widthdrawAmount === ""}
            >
              Withdraw
              <ButtonGlow
                disabled={widthdrawAmount === "0" || widthdrawAmount === ""}
        ...
        {tabOpen === "history" && (
          <ContentColumn>
            <TableHeadersRow>
              <TableHeader flex={2}>Date</TableHeader>
            ...
            <TableContent>
              {props.history &&
                props.history.map((item, index) => (
                  <TableRow key={index}>
                    <TableRowText flex={2}>
                      {dayjs.unix(Math.round(item.timestamp) / 1000).format("hh:mm:ss - DD-MM-YY")}
                    </TableRowText>
                    <TableRowText>{+item.bet.toFixed(2)} SOL</TableRowText>
                    <TableRowText
                      color={item.winnings > 0 ? "#13FF80" : "#FF0055"}
                    >
